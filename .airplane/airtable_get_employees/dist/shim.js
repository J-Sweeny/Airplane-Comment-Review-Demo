...
// .airplane/airtable_get_employees/shim.js
var import_airplane = __toESM(require_airplane());

// tasks/airtableGetEmployees.js
async function airtableGetEmployees_default(params) {
  const data = new Array();
  var Airtable = require("airtable");
  var base = new Airtable({ apiKey: "keyHTBw85lJO6lxIy" }).base("apposaqkT54mKTSbv");
  const getRecords = async () => {
    const records = await base("Moderators").select({
      fields: ["Name"],
      view: "Grid view"
    }).firstPage();
...
async function main() {
  ...
  try {
    var ret;
    if ("__airplane" in airtableGetEmployees_default) {
      ret = await airtableGetEmployees_default.__airplane.baseFunc(JSON.parse(process.argv[2]));
    } else {
      ret = await airtableGetEmployees_default(JSON.parse(process.argv[2]));
    }
    if (ret !== void 0) {
